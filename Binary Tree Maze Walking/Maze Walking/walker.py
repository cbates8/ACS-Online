...
class Walker:
  ...
  def goto_tile(self, maze, tile_coordinates):  # tile_coordinates = (column, row)
    column = tile_coordinates[0]
    row = tile_coordinates[1]
    self.maze_walker.goto(maze.get_init_x() + (column - 0.5) * maze.get_square_size(), maze.get_init_y() - (row - 0.5) * maze.get_square_size())
