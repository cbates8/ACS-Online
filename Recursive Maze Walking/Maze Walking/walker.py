...
class Walker:
  ...
  def jump_to(self, column, row):
    self.maze_walker.goto(self.maze.get_init_x() + (column - 0.5) * self.step, self.maze.get_init_y() - (row - 0.5) * self.step)
  ...
  def is_tile_visited(self, maze, tile_column, tile_row):
    # tiles outside the maze are "blocked" by definition:
    if tile_column < 1 or tile_row < 1 or tile_column > maze.columns or tile_row > maze.rows:
      return True
    ...
    blocked_tiles = maze.get_blockers()
    #print blocked_tiles
    bridge_tiles = blocked_tiles[maze.bridge_color]
    if [tile_column, tile_row] in bridge_tiles:
      return True
    return False
