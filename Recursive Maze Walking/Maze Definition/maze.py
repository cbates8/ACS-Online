...
class Maze:
  ...
  def __init__(self, columns, rows):
    ...
    maze_width = 400
    maze_height = 400
    screen.setup(width=maze_width, height=maze_height)

    #self.square_size = 0.7 * min(maze_drawer.window_width(), maze_drawer.window_height()) / max(columns, rows)
    self.square_size = 0.7 * min(maze_width, maze_height) / max(columns, rows)
    ...
    self.init_x = -maze_width * 0.35
    #self.init_y = maze_drawer.window_height() * 0.35
    self.init_y = maze_height * 0.35
